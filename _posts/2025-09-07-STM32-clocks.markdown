---
layout: post
title:  "STM32 #6: Clocks, Timers, and another interrupt delay on STM32F1xx with libopencm3"
...
- HSI: High-Speed Internal RC oscillator, which is 8 MHz on an F1 chip, lower accuracy than a HSE crystal.
- LSI: Low-Speed Internal oscillator (Typically 32.768 kHz)
- HSE: High-Speed External crystal/oscillator, which is also typically 8 MHz
- LSE: Low-Speed External oscillator (Can be used to run RTC in low-power).
- PLL: Phase-Locked loop that multiplies either the HSI, or HSE.

External clock sources are typically there because they are more accurate than the cheap internal ones on the chip. WHich can be vital for certain critical timing uses such as USB protocol.
