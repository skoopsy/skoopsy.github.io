...
Okay so we have blown a cannon ball through manny of the abstract layers and you should find yourself staring at bare metal. So if everything inside a microcontroller is binary, why don't we just write binary literals directly using 0b? Enter the hexadecimal.
...
Base 16 means values van be represented in a compact manner, for example if we wanted to set the decmial value: ```40960``` in binary that would be: ```0b1010000000000000```, but in hexadecimal it is ```0xA000```. There are a lot fo zeros in that binary, it would be very easy to miss one out, causing all sorts of chaos leaving you vexed. As you might have guessed the 0x prepend tells C that the value is encoded in hexadecimal.
...
Hexadecimal is not so good for visualising bit masks so you will often see conversions but it is always used to set registers due to it being the widely accepted convetion and much more long standing representation in the C language (since 1989) than the 0b specificer.

Lastly, on the point of expressing bit masks, in production code you would often avoid this all together by defining bits with macros. For example defining a decimal 1 unsigned as we previously have, and the left shifting it to the position you want to create the bit mask and assigning it to a #define like so:

```c
#define USART_CR1_TE (1U << 3) // shifts decimal 1 left by 3 places = 0b1000 = 0x8
```
...
On a side note if PCLK1 = 72 MHz you'll notice in the table that at 115200 baus is exactly 39 + 1/16 producing 0% error.

Now we can ebale the USART, and the TX and RX!
```c
...
Lets produce a small function to write bytes to usart 2:
```c
