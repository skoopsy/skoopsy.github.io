...
For embedded C bitwise operators are the key to understanding how to interact with communications protocols, data packets and registers. So to start a revision on binary:
...
Now we know bit shifts, we can write a function in C that can represent a register in binary. 
