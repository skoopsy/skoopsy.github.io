...
We will do this by using a bit wise OR operator, and rather than using the decimal values we will use direct binary represention using the 0b prefix (Only supported in C23 and greater), so the decimal value 2 = 0b10, 3 = 0b11, 4 = 0b100, etc. If you are using <C23 then convert this to hex representation.
...
Avoid using ```GPIOA->ODR |= ...``` in ISRs or multi-context code. BSRR is better for this. If using libopencm3, the BSRR is utilised by the ```gpio_set()``` and ```gpio_clear()``` functions making them atomic and safe to use. For example if you executed ```c gpio_set(GPIOA, GPIO5)``` it would employ ```c GPIO_BSRR = (0x1u << 5)``` to set bit 5 in the BSRR register. However with your knowledge of bitwise operators now you could do something like set a bunch of pins at the same time with the bit wise OR:
